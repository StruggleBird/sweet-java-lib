...
public class URIs {
	...
	/**
	 * A composite URI can be split into one or more CompositeData object which
	 * each represent the individual URIs that comprise the composite one.
	 */
	...
	/**
	 * Give a URI break off any URI options and store them in a Key / Value
	 * Mapping.
	 *
	 * @param uri
	 ...
	 * represents the mapped entries, return the newly updated URI that contains
	 * the value of the given URI and the appended query value.
	 *
	 * @param uri
	 *            The source URI that will have the Map entries appended as a
	 *            URI query value.
	 * @param queryParameters
	 *            The Key / Value mapping that will be transformed into a URI
	 *            query string.
	 *
	 * @return A new URI value that combines the given URI and the constructed
	 *         query string.
	 * @throws URISyntaxException
	 */
	public static URI applyParameters(URI uri,
			Map<String, String> queryParameters) throws URISyntaxException {
	 ...
	 * Given a Key / Value mapping create and append a URI query value that
	 * represents the mapped entries, return the newly updated URI that contains
	 * the value of the given URI and the appended query value. Each entry in
	 * the query string is prefixed by the supplied optionPrefix string.
	 *
	 * @param uri
	 *            The source URI that will have the Map entries appended as a
	 *            URI query value.
	 ...
	 */
	public static URI applyParameters(URI uri,
			Map<String, String> queryParameters, String optionPrefix)
			throws URISyntaxException {
		if (queryParameters != null && !queryParameters.isEmpty()) {
			StringBuffer newQuery = uri.getRawQuery() != null ? new StringBuffer(
			...
			for (Map.Entry<String, String> param : queryParameters.entrySet()) {
				if (param.getKey().startsWith(optionPrefix)) {
					...
					final String key = param.getKey().substring(
							optionPrefix.length());
	...
	/**
	 * Removes any URI query from the given uri and return a new URI that does
	 * not contain the query portion.
	 *
	 * @param uri
	...
	/**
	 * Creates a URI with the given query, removing an previous query value from
	 * the given URI.
	 *
	 * @param uri
	 ...
	 *
	 * @return a new CompsiteData instance representing the parsed composite
	 *         URI.
	 * @throws URISyntaxException
	 */
	 ...
	 * @param first
	 *            The index in the string of the opening parend whose close
	...
	public static int indexOfParenthesisMatch(String str, int first)
		...
		if (first < 0 || first > str.length()) {
			throw new IllegalArgumentException(
					"Invalid position for first parenthesis: " + first);
		}
	...
	/**
	 * Given a composite URI and a CompositeData instance and the scheme
	 * specific part extracted from the source URI, parse the composite URI and
	 * populate the CompositeData object with the results. The source URI is
	 * used only for logging as the ssp should have already been extracted from
	 * it and passed here.
	 *
	 * @param uri
	 ...
	 *            data.
	 * @param rc
	 *            The CompsositeData instance that will be populated from the
	 *            given ssp.
	 * @param ssp
	...
	/**
	 * Given the inner portion of a composite URI, split and return each inner
	 * URI as a string element in a new String array.
	 *
	 * @param str
	 ...
	 *
	 * @param value
	 *            The string that should be trimmed of the given prefix if
	 *            present.
	 * @param prefix
	 *            The prefix to remove from the target string.
	 *
	 * @return either the original string or a new string minus the supplied
	 *         prefix if present.
	 */
	...
	/**
	 * Given a key / value mapping, create and return a URI formatted query
	 * string that is valid and can be appended to a URI.
	 *
	 * @param options
	 ...
	 * portion of the query options needs to be removed and replaced with those
	 * that remain so that other parts of the code can attempt to apply the
	 * remainder or give an error is unknown values were given. This method is
	 * used to update a URI with those remainder values.
	 *
	 * @param originalURI
	 *            The URI whose current parameters are remove and replaced with
	 *            the given remainder value.
	 * @param params
	 *            The URI params that should be used to replace the current ones
	 *            in the target.
	 *
	...
	/**
	 * Given a URI value create and return a new URI that matches the target one
	 * but with the scheme value supplied to this method.
	 *
	 * @param bindAddr
	...
	/**
	 * Examine the supplied string and ensure that all parends appear as
	 * matching pairs.
	 *
	 * @param str
